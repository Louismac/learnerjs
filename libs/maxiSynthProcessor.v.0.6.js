...
class MaxiInstrumentsProcessor extends AudioWorkletProcessor {
   ...
   constructor(options) {
    ...
    this.port.onmessage = (event) => {
      ...
      if(event.data.loopAllEnd !== undefined)
      {
        const start = event.data.loopAllStart;
        const end = event.data.loopAllEnd;
        const beatLength = 60 / this.myClock.bpm;
        const endInSamples = (end / 24) * beatLength * 44100;
        const startInSamples = (start / 24) * beatLength * 44100;
        this.getInstruments().forEach((s)=> {
          s.loopTicks = end;
          s.loopStartTicks = start;
          s.loopStartSamples = startInSamples;
          s.loopSamples = endInSamples;
          s.handleLoop()
        })
      }
      if(event.data.loop !== undefined)
      {
        ...
        const endInSamples = (end / 24) * beatLength * 44100;
        const startInSamples = (start / 24) * beatLength * 44100;
        this.instruments[data.instrument][data.index].loopTicks = end;
        this.instruments[data.instrument][data.index].loopSamples = endInSamples;
        this.instruments[data.instrument][data.index].loopStartTicks = start;
        this.instruments[data.instrument][data.index].loopStartSamples = startInSamples;
        this.instruments[data.instrument][data.index].handleLoop()
      }
    }
  }
