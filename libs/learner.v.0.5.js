  ...
  setupRegressionModel(dataset) {
    ...
    return new Promise((resolve, reject)=> {
      if(typeof tf === "undefined") {
        ...
        link.src = "https://cdn.jsdelivr.net/npm/@tensorflow/tfjs";
        head.appendChild(link);
        console.log("loading tfjs (tfjs@3.0.0)")
        link.onload = ()=> {
          makeModel();
          console.log("loaded tfjs")
          resolve();
        }
      } else {
        console.log("tfjs already loaded")
        makeModel();
        resolve();
      }
    })
  ...
  */
  newExample(input, y = []) {
    ...
    else if(this.running)
    {
      ...
      if(this.modelType == 1)
      {
        let x = this.normalise(tf.tensor([input]))
        const prediction = this.myModel.predict(x)
        this.runEnd(prediction.dataSync());
      }
      else if(this.USE_WORKER)
      {
        if(this.modelType == 2)
        {
  ...
  normalise(x) {
    return x = x.sub(x.min()).div(x.max().sub(x.min()))
  }
